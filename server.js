...
app.get("/dashboards/superadmin" , (req , res) => {
...
app.post('/login-superadmin', async (req, res) => {
    ...
    const superadmin = await User.findOne({ email: 'superadmin@purchase.com', role: 'Superadmin' });
    if (!superadmin) {
    ...
    const token = jwt.sign({ userId: superadmin._id, role: superadmin.role.name }, process.env.JWT_SECRET, { expiresIn: '1h' });
